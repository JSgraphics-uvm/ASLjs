...
function setup() {
  ...
  box.size = box_size;
  box.position = createVector(
    random(windowWidth - 2 * box_size) + box_size,
    random(windowHeight - 2 * box_size) + box_size
  );
  box.digit = 0;
  box.draw = function() {
    push();
...
function draw() {
  ...
  for (let vehicle of vehicles) {
    ...
    for (let box of digit_boxes) {
      force = createVector(
        box.position.x - vehicle.position.x,
        box.position.y - vehicle.position.y
      );
